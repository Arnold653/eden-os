          ...
          <Card style={{ marginBottom: 14 }}>
            <div style={{ display: 'grid', gap: 8 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                {editingObjId && <button onClick={resetObjForm} style={{ background: '#fff', border: `1px solid ${C.line}`, borderRadius: 10, padding: '0 14px', fontWeight: 700, fontSize: 13, cursor: 'pointer', color: C.ink }}>Annuler</button>}
              </div>
            </div>
          </Card>
...
// ---------- Croissance ----------
function CroissanceTab({ settings, lectures, saveLectures, lectureLogs, saveLectureLogs, growthLogs, saveGrowthLogs }) {
                  ...
                  <Card key={l.id}>
                    <div style={{ display: 'grid', gap: 8 }}>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        ...
                        <button onClick={() => setEditingLectureId(null)} style={{ background: '#fff', border: `1px solid ${C.line}`, borderRadius: 10, padding: '0 14px', fontWeight: 700, fontSize: 13, cursor: 'pointer', color: C.ink }}>Annuler</button>
                      </div>
                    </div>
                  </Card>
                );
...
function GrowthNotesJournal({ habit, growthLogs, saveGrowthLogs }) {
  ...
  return (
    <div style={{ marginTop: 8 }}>
      <div style={{ display: 'flex', gap: 6 }}>
        <TextInput placeholder={habit.unit} type="number" value={val} onChange={e => setVal(e.target.value)} style={{ maxWidth: 110 }} />
        <TextInput placeholder="Note : ce qui a été concrètement pratiqué…" value={note} onChange={e => setNote(e.target.value)} style={{ flex: 1 }} />
        <button onClick={addEntry} disabled={!val && !note} style={{ background: C.purple, color: '#fff', border: 'none', borderRadius: 8, padding: '0 14px', fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>+ Ajouter</button>
      </div>
      {avecNote.length > 0 && (
        <button onClick={() => setShowHist(!showHist)} style={{ marginTop: 8, background: 'none', border: `1px solid ${C.line}`, color: C.fade, borderRadius: 8, padding: '5px 10px', fontSize: 11, fontWeight: 700, cursor: 'pointer', width: '100%' }}>
...
function RelationsTab({ settings, contacts, saveContacts, relationLogs, saveRelationLogs }) {
  const [editingId, setEditingId] = useState(null);
  ...
  function removeContact(id) { saveContacts(contacts.filter(c => c.id !== id)); if (editingId === id) resetForm(); }
...
// ---------- Paramètres ----------
function ParametresPanel({ settings, saveSettings, onClose, onExport, onImport, onChangeCurrency, userEmail }) {
        ...
        <Card style={{ marginBottom: 18 }}>
          ...
          {confirmCurrency && (
            <div style={{ background: C.cream, border: `1px solid ${C.gold}`, borderRadius: 10, padding: 12, marginTop: 10 }}>
              <div style={{ fontSize: 12.5, color: C.navy, marginBottom: 8 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => setConfirmCurrency(null)} style={{ flex: 1, background: '#fff', border: `1px solid ${C.line}`, borderRadius: 8, padding: '9px', fontWeight: 700, fontSize: 12.5, cursor: 'pointer', color: C.ink }}>Annuler</button>
        ...
        <Card style={{ marginBottom: 18 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <div style={{ fontSize: 13, fontWeight: 700, color: C.navy }}>{userEmail || '—'}</div>
            </div>
        ...
        <Card style={{ marginBottom: 18 }}>
          ...
          {!pendingImport ? (
            <label style={{
              ...
              background: '#fff', border: `1px solid ${C.line}`, borderRadius: 10, padding: '10px 14px', fontWeight: 700, fontSize: 13, color: C.navy,
            }}>
            ...
            <div style={{ background: C.cream, border: `1px solid ${C.gold}`, borderRadius: 10, padding: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => setPendingImport(null)} style={{ flex: 1, background: '#fff', border: `1px solid ${C.line}`, borderRadius: 8, padding: '9px', fontWeight: 700, fontSize: 12.5, cursor: 'pointer', color: C.ink }}>Annuler</button>
    ...
    <div style={{ position: 'absolute', inset: 0, zIndex: 25 }}>
      ...
      <div style={{
        ...
        <div style={{ padding: '10px 16px 4px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontFamily: FONT_DISPLAY, fontSize: 17, fontWeight: 700, color: C.navy }}>Modules</span>
        ...
        <div style={{ padding: 16, overflowY: 'auto' }}>
          {visibleCapitals.map(cap => {
            ...
            return (
              <div key={cap.id} style={{ marginBottom: 10, border: `1px solid ${C.line}`, borderRadius: 14, overflow: 'hidden', background: '#fff' }}>
                <button
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                    ...
                    <span style={{ fontSize: 13.5, fontWeight: 700, color: C.navy, letterSpacing: 0.2 }}>{cap.name}</span>
                  </div>
...
export default function App() {
  ...
  function exportData() {
    const data = {
      ...
      growthLogs, contacts, disciplineSubjects, relationLogs,
    };
  ...
  async function importData(file) {
    try {
      ...
      const setters = {
        ...
        growthLogs: setGrowthLogs, contacts: setContacts, disciplineSubjects: setDisciplineSubjects, relationLogs: setRelationLogs,
      };
  ...
  return (
    <div style={{ background: C.cream, minHeight: '100vh', fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ maxWidth: 460, margin: '0 auto', minHeight: '100vh', background: '#fff', boxShadow: '0 0 30px rgba(0,0,0,0.08)', position: 'relative', paddingBottom: 76 }}>
        <Header monthIdx={monthIdx} year={year} onPrev={prevMonth} onNext={nextMonth} tauxDime={tauxDime} onOpenSettings={() => setShowSettings(true)} showBack={tab !== 'dashboard'} onBack={goBack} />
        <div style={{ padding: '16px 16px 8px' }}>
          ...
          {tab === 'transactions' && <TransactionsTab settings={settings} monthTx={monthTx} addTransaction={addTransaction} updateTransaction={updateTransaction} duplicateTransaction={duplicateTransaction} deleteTransaction={deleteTransaction} groupTotals={groupTotals} />}
          ...
          {tab === 'provisions' && <ProvisionsTab settings={settings} provisions={provisions} saveProvisions={saveProvisions} monthIdx={monthIdx} />}
          {tab === 'dettes' && <DettesTab settings={settings} debts={debts} saveDebts={saveDebts} />}
          ...
          {tab === 'croissance' && <CroissanceTab settings={settings} lectures={lectures} saveLectures={saveLectures} lectureLogs={lectureLogs} saveLectureLogs={saveLectureLogs} growthLogs={growthLogs} saveGrowthLogs={saveGrowthLogs} />}
          {tab === 'relations' && <RelationsTab settings={settings} contacts={contacts} saveContacts={saveContacts} relationLogs={relationLogs} saveRelationLogs={saveRelationLogs} />}
        ...
        {showSettings && <ParametresPanel settings={settings} saveSettings={saveSettings} onClose={() => setShowSettings(false)} onExport={exportData} onImport={importData} onChangeCurrency={changeCurrency} userEmail={userEmail} />}
