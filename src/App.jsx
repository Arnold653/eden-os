...
function FoyerSection() {
  ...
  async function refresh() {
    ...
    try {
      ...
      const { data: membership } = await supabase.from('foyer_members').select('foyer_id').eq('user_id', user.id).maybeSingle();
      if (!membership) { setState({ none: true }); return; }
      const { data: foyer } = await supabase.from('foyers').select('join_code').eq('id', membership.foyer_id).maybeSingle();
      const { count } = await supabase.from('foyer_members').select('*', { count: 'exact', head: true }).eq('foyer_id', membership.foyer_id);
      setState({ foyerId: membership.foyer_id, joinCode: foyer?.join_code, memberCount: count || 1 });
    } catch (e) {
      setState({ none: true });
    }
  }
