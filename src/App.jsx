...
// Bouton de dictée vocale (Web Speech API du navigateur — gratuit, sans compte).
function MicButton({ onResult, size = 15 }) {
  const [listening, setListening] = useState(false);
  function start() {
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!SpeechRecognition) { window.alert("La dictée vocale n'est pas prise en charge sur ce navigateur."); return; }
  ...
  return (
    <button type="button" onClick={start} title="Dicter au micro" style={{
      background: listening ? C.terracotta : C.cream, color: listening ? '#fff' : C.gold, border: 'none', borderRadius: 8,
      width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0,
    }}><Mic size={size} /></button>
  );
}
...
function computeStreak(logs, idField, id) {
  const dates = new Set(logs.filter(l => l[idField] === id && Number(l.value ?? l.hours ?? 0) > 0).map(l => l.date));
...
function pickIcon(name) {
  ...
  if (n.includes('famille')) return Users;
  if (n.includes('travail')) return Clock;
  if (n.includes('repos') || n.includes('sommeil') || n.includes('dormir')) return Moon;
  if (n.includes('sport') || n.includes('exercice') || n.includes('activité')) return HeartPulse;
  if (n.includes('poids')) return Gauge;
  if (n.includes('eau') || n.includes('hydrat')) return Droplet;
  if (n.includes('service') || n.includes('don')) return HandCoins;
  if (n.includes('compétence') || n.includes('competence')) return Gauge;
...
// ---------- Header ----------
function Header({ tab, monthIdx, year, onPrev, onNext, tauxDime, onOpenSettings, onOpenSearch, showBack, onBack, unreadCount, onOpenNotifCenter }) {
  if (!showBack) {
    return (
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            <button onClick={onOpenNotifCenter} style={{ position: 'relative', background: 'rgba(255,255,255,0.10)', border: '1px solid rgba(255,255,255,0.14)', borderRadius: 9, padding: 8, color: '#fff', cursor: 'pointer' }}>
              <Bell size={17} />
              {unreadCount > 0 && (
                <span style={{ position: 'absolute', top: -4, right: -4, background: C.terracotta, color: '#fff', borderRadius: 999, minWidth: 16, height: 16, fontSize: 9.5, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 3px' }}>
  ...
  // Écrans de module : titre contextuel, sélecteur mois/année uniquement si pertinent.
  let moduleInfo = null;
  for (const cap of CAPITALS) {
    const m = cap.modules.find(mm => mm.id === tab);
    if (m) { moduleInfo = m; break; }
  }
  const MONTH_SCOPED = tab === 'transactions' || tab === 'royaume';
  const ModIcon = moduleInfo?.icon || Compass;
  return (
    <div style={{
      background: `linear-gradient(160deg, ${C.navy} 0%, #072238 100%)`, color: '#fff', padding: '16px',
      boxShadow: 'inset 0 -1px 0 rgba(255,255,255,0.06)',
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <button onClick={onBack} style={{ background: 'rgba(255,255,255,0.10)', border: '1px solid rgba(255,255,255,0.14)', borderRadius: 9, padding: 8, color: '#fff', cursor: 'pointer', flexShrink: 0 }}>
          <ChevronLeft size={17} />
        </button>
        <ModIcon size={19} color={C.goldLight} style={{ flexShrink: 0 }} />
        <span style={{ fontFamily: FONT_DISPLAY, fontSize: 18, fontWeight: 700 }}>{moduleInfo?.label || 'Eden OS'}</span>
      </div>
      {MONTH_SCOPED && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 14, marginTop: 12 }}>
          <button onClick={onPrev} style={{ background: 'rgba(255,255,255,0.10)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 7, padding: 4, color: '#fff', cursor: 'pointer' }}><ChevronLeft size={16} /></button>
...
const DAILY_THOUGHTS = [
  { ref: 'Proverbes 3:9-10', text: "Honorer Dieu avec ce que l'on possède, en premier et non en dernier, est un acte de confiance plus que de calcul." },
  { ref: 'Malachie 3:10', text: "La dîme n'est pas un impôt spirituel : c'est une invitation à vérifier, dans les faits, si l'on croit vraiment ce que l'on dit croire." },
  { ref: 'Luc 16:10', text: "Celui qui est fidèle dans les petites choses l'est aussi dans les grandes — la rigueur du quotidien n'est jamais un détail." },
  { ref: 'Proverbes 21:5', text: "Les projets de l'homme diligent ne mènent qu'à l'abondance ; la précipitation ne mène qu'au manque." },
  { ref: 'Ecclésiaste 3:1', text: "Il y a un temps pour chaque chose — planifier son temps n'est pas une contrainte, c'est reconnaître que chaque saison mérite d'être vécue pleinement." },
  { ref: 'Proverbes 27:23', text: "Connais bien l'état de tes troupeaux — l'intendance commence par regarder honnêtement ce qui est déjà là." },
  { ref: 'Romains 13:8', text: "Ne devoir rien à personne, sinon l'amour — le désendettement n'est pas qu'une stratégie financière, c'est une forme de liberté." },
  { ref: '1 Timothée 5:8', text: "Pourvoir aux besoins des siens n'est pas optionnel — c'est l'un des premiers fruits visibles d'une foi authentique." },
  { ref: 'Jacques 1:5', text: "La sagesse se demande — humblement, souvent, sans honte de ne pas déjà tout savoir." },
  { ref: 'Proverbes 15:22', text: "Les projets échouent faute de conseil ; ils réussissent par le nombre des conseillers." },
  { ref: 'Psaume 90:12', text: "Apprendre à compter ses jours, c'est acquérir un cœur de sagesse — le temps compté est un temps mieux habité." },
  { ref: 'Proverbes 13:11', text: "La richesse amassée peu à peu s'accroît ; celle qui vient vite s'évapore aussi vite." },
  { ref: '2 Corinthiens 9:7', text: "Donner sans contrainte ni regret — la générosité forcée n'en est déjà plus une." },
  { ref: 'Proverbes 22:6', text: "Former un enfant selon la voie qu'il doit suivre est un investissement dont la récolte dépasse largement le temps qu'on y consacre." },
  { ref: 'Galates 6:9', text: "Ne pas se lasser de bien faire — la persévérance récolte toujours plus tard que prévu, mais elle récolte." },
  { ref: 'Proverbes 16:3', text: "Recommande à Dieu tes projets, et ils réussiront — la planification n'exclut pas la dépendance, elle la suppose." },
  { ref: 'Matthieu 6:21', text: "Là où est ton trésor, là aussi sera ton cœur — regarder ses dépenses, c'est parfois se regarder soi-même." },
  { ref: 'Proverbes 11:14', text: "Sans conseil, un peuple tombe ; le salut est dans le grand nombre de conseillers." },
  { ref: 'Hébreux 10:24-25', text: "S'encourager mutuellement, ne pas déserter ses relations — la fidélité relationnelle se construit aussi dans la régularité." },
  { ref: 'Proverbes 27:17', text: "Le fer aiguise le fer ; un ami aiguise le caractère de son ami — les relations choisies façonnent qui l'on devient." },
  { ref: '1 Corinthiens 4:2', text: "Ce que l'on demande à un intendant, avant tout, c'est d'être trouvé fidèle — pas parfait, pas brillant : fidèle." },
  { ref: 'Proverbes 24:27', text: "Mets d'abord en ordre ton travail dehors, puis bâtis ta maison — l'ordre des priorités change souvent le résultat final." },
  { ref: '3 Jean 1:2', text: "Que tu prospères à tous égards et sois en bonne santé, comme prospère ton âme — le corps et l'âme ne sont jamais vraiment séparés." },
  { ref: 'Proverbes 6:6-8', text: "Observe la fourmi : sans chef, sans contrôleur, elle prépare sa nourriture en été — la discipline n'attend pas la surveillance." },
  { ref: 'Philippiens 4:11-12', text: "Apprendre à être content en toute situation n'est pas un renoncement — c'est une liberté intérieure qui ne dépend plus des circonstances." },
];
...
function Dashboard({ revenuMois, donsMois, depensesMois, soldeMois, tauxDime, pieData, last6, comptesSoldes, objectifZero, indice, settings, transactions, debts, timeLogs, healthLogs, disciplineLogs, decisions, objectifs, revues, provisions, lectures, lectureLogs, growthLogs, monthIdx, year }) {
  ...
  const objectifsActifsMois = (objectifs || []).filter(o => o.statut !== 'abandonné');

  function exportMonthlyPDF() {
    const doc = new jsPDF();
    const marginLeft = 14, pageW = 196;
    let y = 22;
    const pageH = doc.internal.pageSize.height;
    function ensureSpace(n = 1) { if (y + n * 7 > pageH - 15) { doc.addPage(); y = 22; } }
    function h1(text) {
      ensureSpace(2);
      doc.setFontSize(15); doc.setFont(undefined, 'bold'); doc.setTextColor(11, 47, 78);
      doc.text(text, marginLeft, y); y += 4;
      doc.setDrawColor(201, 138, 43); doc.setLineWidth(0.6); doc.line(marginLeft, y, pageW, y); y += 8;
    }
    function row(label, value) {
      ensureSpace(1);
      doc.setFontSize(10); doc.setFont(undefined, 'normal'); doc.setTextColor(40, 40, 40);
      doc.text(String(label), marginLeft, y);
      doc.text(String(value), pageW, y, { align: 'right' });
      y += 6.2;
    }
    function note(text) {
      const lines = doc.splitTextToSize(text, pageW - marginLeft);
      ensureSpace(lines.length);
      doc.setFontSize(9.5); doc.setTextColor(110, 110, 110);
      doc.text(lines, marginLeft, y); y += lines.length * 4.6 + 3;
    }

    doc.setFontSize(20); doc.setFont(undefined, 'bold'); doc.setTextColor(11, 47, 78);
    doc.text('Eden OS — Rapport mensuel', marginLeft, y); y += 8;
    doc.setFontSize(12); doc.setFont(undefined, 'normal'); doc.setTextColor(90, 90, 90);
    doc.text(`${MONTHS_FR[monthIdx]} ${year}`, marginLeft, y); y += 10;

    h1("Indice d'Intendance");
    row('Score global', `${Math.round(indice.total)} / 100 — ${indice.niveau}`);
    (indice.dims || []).filter(d => !((indice.disabledDims||[]).includes(d.key))).forEach(d => {
      row(d.label, d.score === null ? 'saisie manuelle' : `${Math.round(d.score)} / 100`);
    });
    y += 4;

    if (isModuleVisible(settings, 'transactions')) {
      h1('Finances');
      row('Revenu du mois', fmt(revenuMois));
      row('Dépenses', fmt(depensesMois));
      row('Solde', fmt(soldeMois));
      if (isModuleVisible(settings, 'royaume')) row('Taux de dîme', pct(tauxDime));
      if (pieData && pieData.length) {
        y += 2; doc.setFontSize(10); doc.setFont(undefined, 'bold'); doc.setTextColor(11,47,78);
        ensureSpace(1); doc.text('Répartition des dépenses par groupe', marginLeft, y); y += 6;
        pieData.forEach(g => row(g.name, fmt(g.value)));
      }
      y += 4;
    }

    if (isModuleVisible(settings, 'dettes') && (debts||[]).length > 0) {
      h1('Dettes');
      debts.forEach(d => row(d.name, `${fmt(d.currentBalance)} restant · ${fmt(d.monthlyPayment)}/mois`));
      y += 4;
    }

    if (isModuleVisible(settings, 'provisions') && provisionsActives.length > 0) {
      h1('Provisions');
      row('Provisions à jour', `${provisionsAJour} / ${provisionsActives.length}`);
      row('Total réservé', fmt(totalReserve));
      y += 4;
    }

    if (isModuleVisible(settings, 'temps')) {
      h1('Temps');
      row('Heures suivies cette semaine', `${totalHeuresSemaine.toFixed(1)} h`);
      if (catEnRetard) row('Catégorie la plus en retard', `${catEnRetard.name} (${catEnRetard.total.toFixed(1)}/${catEnRetard.weeklyTarget} h)`);
      y += 4;
    }

    if (isModuleVisible(settings, 'royaume')) {
      h1('Disciplines spirituelles');
      row('Progression moyenne du mois', discMoyenne !== null ? pct(discMoyenne) : 'aucune donnée');
      y += 4;
    }

    if (isModuleVisible(settings, 'sagesse')) {
      h1('Sagesse');
      row('Décisions ce mois-ci', String(monthDecisions.length));
      monthDecisions.slice(0, 8).forEach(d => note(`• ${d.objet}${d.majeure ? ' (majeure)' : ''}`));
      y += 2;
    }

    if (isModuleVisible(settings, 'vision')) {
      h1('Vision');
      row('Objectifs actifs', String(objectifsActifsMois.length));
      objectifsActifsMois.slice(0, 8).forEach(o => note(`• ${o.titre} — ${o.statut}`));
      y += 2;
    }

    if (isModuleVisible(settings, 'croissance')) {
      h1('Croissance');
      row('Pages lues cette semaine', String(pagesSemaine));
      row('Lecture en cours', lectureEnCours ? lectureEnCours.titre : '—');
      if (growthMoyenne !== null) row('Habitudes — progression moyenne', pct(growthMoyenne));
    }

    doc.save(`eden-os-rapport-${MONTHS_FR[monthIdx].toLowerCase()}-${year}.pdf`);
  }
  ...
  return (
    <div>
      ...
      {subview === 'mois' && (
      <>
      {(() => { const thought = getDailyThought(); return (
        <Card style={{ marginBottom: 14, background: `linear-gradient(135deg, ${C.cream} 0%, #fff 100%)`, border: `1px solid ${C.gold}` }}>
          <div style={{ fontSize: 9.5, fontWeight: 800, color: C.gold, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 5 }}>Pensée du jour</div>
          <div style={{ fontSize: 13, color: C.ink, fontStyle: 'italic', lineHeight: 1.4 }}>{thought.text}</div>
          <div style={{ fontSize: 10.5, color: C.fade, marginTop: 7, textAlign: 'right' }}>{thought.ref}</div>
        </Card>
      ); })()}
      ...
      {isModuleVisible(settings, 'croissance') && (
        ...
        <button onClick={exportMonthlyPDF} style={{
          width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
          background: C.navy, color: '#fff', border: 'none', borderRadius: 10, padding: '12px', fontWeight: 700, fontSize: 13.5, cursor: 'pointer',
        }}><Download size={16} /> Exporter le rapport du mois (PDF)</button>
        </>
      )}
      </>
      )}
        ...
        <div>
          ...
          <div style={{ display: 'grid', gap: 10 }}>
            {settings.disciplines.map(d => {
              ...
              const streak = computeStreak(disciplineLogs, 'disciplineId', d.id);
                  ...
                  <div onClick={() => setExpandedDiscId(isOpen ? null : d.id)} style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                        ...
                        {streak >= 2 && (
                          <span style={{ display: 'flex', alignItems: 'center', gap: 2, background: C.cream, borderRadius: 10, padding: '1px 6px' }}>
                            <Flame size={11} color={C.terracotta} /><span style={{ fontSize: 10, fontWeight: 800, color: C.terracotta }}>{streak}</span>
                          </span>
                        )}
                      </div>
        ...
        <div style={{ marginBottom: 10 }}>
          ...
          {exauces.map(s => (
            <div key={s.id} style={{ padding: '7px 0', borderBottom: `1px solid ${C.line}` }}>
              ...
              <div style={{ display: 'flex', gap: 6, marginTop: 4 }}>
                <TextInput placeholder="Témoignage : ce qui a été reçu…" value={noteDraft[s.id] ?? s.note} onChange={e => setNoteDraft({ ...noteDraft, [s.id]: e.target.value })} onBlur={() => saveNote(s.id)} style={{ fontSize: 12, flex: 1 }} />
                <MicButton size={13} onResult={t => { const v = (noteDraft[s.id] ?? s.note) || ''; const nv = v ? v + ' ' + t : t; setNoteDraft({ ...noteDraft, [s.id]: nv }); saveDisciplineSubjects(disciplineSubjects.map(x => x.id === s.id ? { ...x, note: nv } : x)); }} />
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div style={{ marginTop: 4 }}>
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
        ...
        <button onClick={addEntry} disabled={!val && !note} style={{ flex: 1, background: C.purple, color: '#fff', border: 'none', borderRadius: 8, padding: '9px 14px', fontWeight: 700, fontSize: 13, cursor: (!val && !note) ? 'default' : 'pointer', opacity: (!val && !note) ? 0.5 : 1 }}>+ Ajouter au journal</button>
      </div>
      {avecNote.length > 0 && (
        <button onClick={() => setShowHist(!showHist)} style={{ marginTop: 10, background: 'none', border: `1px solid ${C.line}`, color: C.fade, borderRadius: 8, padding: '6px 10px', fontSize: 11, fontWeight: 700, cursor: 'pointer', width: '100%' }}>
          ...
          return (
            <Card key={d.id}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <RingProgress value={progress} size={34} strokeWidth={3.5} color={C.green} />
                  <div style={{ display: 'flex', gap: 2 }}>
...
function VisionTab({ visionDoc, saveVisionDoc, objectifs, saveObjectifs, revues, saveRevues, onTrash }) {
  ...
  function resetWizard() { setWizardStep(1); }

  function resetObjForm() { setEditingObjId(null); setTitre(''); setDescription(''); setEcheance(''); setWizardStep(1); }
    ...
    <div>
      ...
      {subview === 'objectifs' && (
        <div>
          ...
          {!editingObjId && (
            <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
              <button onClick={() => { setWizardMode(true); resetWizard(); }} style={{ flex: 1, padding: '8px', borderRadius: 8, border: `1px solid ${C.line}`, fontWeight: 700, fontSize: 12, cursor: 'pointer', background: wizardMode ? C.navy : C.surface, color: wizardMode ? '#fff' : C.ink }}>Assistant guidé</button>
              <button onClick={() => setWizardMode(false)} style={{ flex: 1, padding: '8px', borderRadius: 8, border: `1px solid ${C.line}`, fontWeight: 700, fontSize: 12, cursor: 'pointer', background: !wizardMode ? C.navy : C.surface, color: !wizardMode ? '#fff' : C.ink }}>Formulaire rapide</button>
            </div>
          )}

          {wizardMode && !editingObjId ? (
            <Card style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 10.5, color: C.fade, marginBottom: 10 }}>Étape {wizardStep} sur 3</div>

              {wizardStep === 1 && (
                <>
                  <div style={{ fontSize: 14, fontWeight: 700, color: C.heading, marginBottom: 10 }}>Sur quel horizon te projettes-tu ?</div>
                  <div style={{ display: 'grid', gap: 8 }}>
                    {HORIZONS.map(h => (
                      <button key={h.id} onClick={() => { setHorizon(h.id); setWizardStep(2); }} style={{
                        textAlign: 'left', padding: '12px 14px', borderRadius: 10, border: `1px solid ${horizon===h.id ? C.gold : C.line}`, cursor: 'pointer',
                        background: horizon===h.id ? C.cream : C.surface, fontWeight: 700, fontSize: 13, color: C.ink,
                      }}>{h.label}</button>
                    ))}
                  </div>
                </>
              ...
              {wizardStep === 2 && (
                <>
                  <div style={{ fontSize: 14, fontWeight: 700, color: C.heading, marginBottom: 10 }}>Que veux-tu accomplir précisément, et d'ici quand ?</div>
                  <div style={{ display: 'grid', gap: 8 }}>
                    <TextInput placeholder="Ex : Solder mes deux dettes de départ" value={titre} onChange={e => setTitre(e.target.value)} />
                    <div>
                      <div style={{ fontSize: 10.5, color: C.fade, marginBottom: 3 }}>Échéance</div>
                      <TextInput type="date" value={echeance} onChange={e => setEcheance(e.target.value)} />
                    </div>
                    <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                      <button onClick={() => setWizardStep(1)} style={{ background: C.surface, border: `1px solid ${C.line}`, borderRadius: 10, padding: '10px 14px', fontWeight: 700, fontSize: 13, cursor: 'pointer', color: C.ink }}>Retour</button>
                      <PrimaryButton onClick={() => setWizardStep(3)} disabled={!titre} style={{ flex: 1 }}>Continuer</PrimaryButton>
                    </div>
                  </div>
                </>
              )}
              ...
              {wizardStep === 3 && (
                <>
                  <div style={{ fontSize: 14, fontWeight: 700, color: C.heading, marginBottom: 4 }}>Pourquoi cet objectif compte pour toi ?</div>
                  <div style={{ fontSize: 11.5, color: C.fade, marginBottom: 10 }}>Un objectif tient dans la durée quand il est relié à ta Vision et à ton Appel, pas seulement à une envie du moment.</div>
                  <div style={{ display: 'grid', gap: 8 }}>
                    <TextInput placeholder="Ce que ça changera, ce à quoi ça te relie…" value={description} onChange={e => setDescription(e.target.value)} />
                    <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                      <button onClick={() => setWizardStep(2)} style={{ background: C.surface, border: `1px solid ${C.line}`, borderRadius: 10, padding: '10px 14px', fontWeight: 700, fontSize: 13, cursor: 'pointer', color: C.ink }}>Retour</button>
                      <PrimaryButton onClick={() => { addObjectif(); setWizardStep(1); }} style={{ flex: 1 }}><Plus size={15}/> Créer l'objectif</PrimaryButton>
                    </div>
                  </div>
                </>
              )}
    ...
    <div>
      ...
      <div style={{ display: 'grid', gap: 10 }}>
        {(settings.timeCategories || []).map(c => {
          ...
          return (
            <Card key={c.id}>
              <div onClick={() => setExpandedCatId(isOpen ? null : c.id)} style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
                <div style={{ width: 40, height: 40, borderRadius: 12, background: C.cream, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <Icon size={19} color={c.color} />
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: c.color }}>{c.name}</div>
                  <div style={{ fontSize: 11.5, color: C.fade }}>{total.toFixed(1)} / {c.weeklyTarget} h</div>
                </div>
                <RingProgress value={progress} size={38} color={c.color} />
                <ChevronDown size={16} color={C.fade} style={{ transform: isOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s', flexShrink: 0 }} />
              </div>
              {isOpen && (
                <div style={{ marginTop: 12, borderTop: `1px solid ${C.line}`, paddingTop: 12, display: 'flex', gap: 6 }}>
    ...
    <div>
      ...
      <div style={{ display: 'grid', gap: 10, marginBottom: 14 }}>
        {(settings.healthMetrics || []).map(m => {
          ...
          const Icon = pickIcon(m.name);
          return (
            <Card key={m.id}>
              <div onClick={() => setExpandedMetricId(isOpen ? null : m.id)} style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
                <div style={{ width: 40, height: 40, borderRadius: 12, background: C.cream, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <Icon size={19} color={C.green} />
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 14, fontWeight: 700 }}>{m.name}</div>
                  <div style={{ fontSize: 11.5, color: C.fade }}>{total.toFixed(1)} / {m.weeklyTarget} {m.unit}</div>
                </div>
                <RingProgress value={progress} size={38} color={C.green} />
                <ChevronDown size={16} color={C.fade} style={{ transform: isOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s', flexShrink: 0 }} />
              </div>
              {isOpen && (
                <div style={{ marginTop: 12, borderTop: `1px solid ${C.line}`, paddingTop: 12, display: 'flex', gap: 6 }}>
...
function computeReminders({ settings, disciplineLogs, contacts, decisions, revues }) {
  const reminders = [];
  const today = new Date();
  const todayIso = todayISO();

  // Disciplines silencieuses depuis 3 jours ou plus
  (settings.disciplines || []).forEach(d => {
    const mine = disciplineLogs.filter(l => l.disciplineId === d.id && Number(l.value || 0) > 0);
    if (mine.length === 0) return; // jamais commencée : pas de rappel culpabilisant
    const lastDate = mine.reduce((max, l) => l.date > max ? l.date : max, mine[0].date);
    const days = Math.floor((today - new Date(lastDate)) / 86400000);
    if (days >= 3) reminders.push({ icon: 'discipline', text: `${d.name} : rien noté depuis ${days} jours` });
  });

  // Contacts à recontacter
  (contacts || []).forEach(c => {
    const days = Math.floor((today - new Date(c.dernierContact)) / 86400000);
    if (days > c.frequenceCible) reminders.push({ icon: 'contact', text: `${c.nom} : à recontacter (${days} j, objectif ${c.frequenceCible} j)` });
  });

  // Décisions majeures à relire
  (decisions || []).forEach(d => {
    if (d.majeure && d.dateRelecture && d.dateRelecture <= todayIso && d.statut !== 'relue') {
      reminders.push({ icon: 'decision', text: `Décision « ${d.objet} » : relecture à faire` });
    }
  });

  // Revue trimestrielle Vision en retard (trimestre à plus de moitié écoulé, sans revue)
  const qIdx = Math.floor(today.getMonth() / 3);
  const qStart = new Date(today.getFullYear(), qIdx * 3, 1);
  const daysIntoQuarter = Math.floor((today - qStart) / 86400000);
  const hasRevueThisQuarter = (revues || []).some(r => new Date(r.date) >= qStart && new Date(r.date) <= today);
  if (daysIntoQuarter > 45 && !hasRevueThisQuarter) {
    reminders.push({ icon: 'vision', text: `Revue trimestrielle (T${qIdx + 1}) pas encore faite` });
  }

  return reminders;
}

// ---------- Conseil : bibliothèque d'enseignement + moteur de sélection ----------
// Chaque entrée : condition (fonction des données), gravité (urgent/positif),
// priorité (plus haut = affiché en premier si plusieurs se déclenchent), et texte.
// Le ton : jamais moralisateur, toujours un principe + une orientation concrète.
function computeConseils({ settings, transactions, disciplineLogs, contacts, decisions, revues, provisions, objectifs, lectures, lectureLogs, debts }) {
  const today = new Date();
  const todayIso = todayISO();
  ...
  // --- 1. Dîme ---
  const monthTx = (transactions || []).filter(t => { const d = new Date(t.date); return d.getMonth() === today.getMonth() && d.getFullYear() === today.getFullYear(); });
  const revenuMois = monthTx.filter(t => t.type === 'revenu').reduce((s, t) => s + Number(t.amount || 0), 0);
  const titheIds = (settings.donTypes || []).filter(d => d.isTithe).map(d => d.id);
  const dimeMois = monthTx.filter(t => t.type === 'don' && titheIds.includes(t.donTypeId)).reduce((s, t) => s + Number(t.amount || 0), 0);
  const tauxDimeMois = revenuMois > 0 ? dimeMois / revenuMois : null;
  // tendance sur 3 derniers mois
  let moisSousObjectif = 0;
  for (let i = 0; i < 3; i++) {
    const d = new Date(today.getFullYear(), today.getMonth() - i, 1);
    const tx = (transactions || []).filter(t => { const td = new Date(t.date); return td.getMonth() === d.getMonth() && td.getFullYear() === d.getFullYear(); });
    const rev = tx.filter(t => t.type === 'revenu').reduce((s, t) => s + Number(t.amount || 0), 0);
    const dime = tx.filter(t => t.type === 'don' && titheIds.includes(t.donTypeId)).reduce((s, t) => s + Number(t.amount || 0), 0);
    if (rev > 0 && dime / rev < 0.1) moisSousObjectif++;
  }
  if (moisSousObjectif >= 3) {
    conseils.push({ id: 'dime_bas', priority: 90, tone: 'orientation', category: 'Royaume', title: 'Fidélité dans le peu',
      body: `Ton taux de dîme est à ${tauxDimeMois !== null ? (tauxDimeMois * 100).toFixed(0) : '—'}% ce mois-ci, sous l'objectif de 10% depuis 3 mois. La dîme n'est pas d'abord une question de montant — c'est un acte qui rappelle à qui appartient vraiment ce que tu gères (Psaume 24:1). Avant de chercher à combler l'écart d'un coup, regarde plutôt : y a-t-il une dépense récurrente qui a pris la place qui revenait à Dieu ? Ce mois-ci, propose-toi un petit ajustement, pas un exploit.` });
  } else if (tauxDimeMois !== null && tauxDimeMois >= 0.1 && moisSousObjectif === 0) {
    conseils.push({ id: 'dime_haut', priority: 20, tone: 'encouragement', category: 'Royaume', title: 'Une fidélité qui tient',
      body: `Ton taux de dîme est à ${(tauxDimeMois * 100).toFixed(0)}% et se maintient au-dessus de l'objectif depuis plusieurs mois. Ce n'est pas rien de rester fidèle quand plus rien ne l'exige — c'est là que se construit vraiment le caractère (1 Corinthiens 4:2). Continue, et si l'occasion se présente, songe à ce que "donner au-delà de la dîme" pourrait représenter pour toi.` });
  }
  ...
  (settings.disciplines || []).forEach(d => {
    ...
    const lastDate = mine.reduce((max, l) => l.date > max ? l.date : max, mine[0].date);
    const silence = days(lastDate);
    if (silence >= 5) {
      conseils.push({ id: `discipline_silence_${d.id}`, priority: 70, tone: 'orientation', category: 'Royaume', title: 'Reprendre le fil',
        body: `${d.name} : rien noté depuis ${silence} jours. Une discipline interrompue n'est pas un échec, c'est une invitation à recommencer — "sept fois" s'il le faut (Proverbes 24:16). Ne cherche pas à rattraper les jours manqués : reprends simplement aujourd'hui, à la mesure d'aujourd'hui.` });
    } else {
      // streak
      const datesSet = new Set(mine.map(l => l.date));
      let streak = 0; const cursor = new Date();
      if (!datesSet.has(todayIso)) cursor.setDate(cursor.getDate() - 1);
      while (datesSet.has(cursor.toISOString().slice(0, 10))) { streak++; cursor.setDate(cursor.getDate() - 1); }
      if ([14, 30, 100].includes(streak)) {
        conseils.push({ id: `discipline_streak_${d.id}_${streak}`, priority: 40, tone: 'encouragement', category: 'Royaume', title: 'La régularité qui façonne',
          body: `${d.name} : ${streak} jours sans interruption. La Bible parle peu de grands exploits ponctuels et beaucoup de fidélité répétée (Luc 16:10). Ce que tu viens de construire n'est pas visible dans un seul jour, mais dans la somme des ${streak}. Profite de cette lancée pour te demander : quelle est la prochaine discipline, aujourd'hui négligée, qui mériterait la même constance ?` });
      }
    }
  });

  // --- 3. Décisions ---
  const majeuresEnAttente = (decisions || []).filter(d => d.majeure && d.dateRelecture && d.dateRelecture <= todayIso && d.statut !== 'relue');
  if (majeuresEnAttente.length > 0) {
    const d0 = majeuresEnAttente[0];
    conseils.push({ id: 'decision_non_relue', priority: 65, tone: 'orientation', category: 'Sagesse', title: 'Le discernement a besoin de recul',
      body: `Tu as noté une décision importante (« ${d0.objet} ») il y a ${days(d0.dateRelecture)} jours, sans être revenu dessus. Beaucoup de décisions qui semblaient sages sur le moment se révèlent différentes avec le recul — ce n'est pas un manque de foi que de vérifier, c'est de la sagesse (Proverbes 15:22). Prends 10 minutes cette semaine pour relire ce que tu avais écrit, et demande-toi : est-ce que je referais ce choix aujourd'hui ?` });
  } else {
    const majeures = (decisions || []).filter(d => d.majeure);
    if (majeures.length >= 2 && majeures.every(d => d.statut === 'relue' || !d.dateRelecture || d.dateRelecture > todayIso)) {
      conseils.push({ id: 'decision_a_jour', priority: 15, tone: 'encouragement', category: 'Sagesse', title: 'Un discernement entretenu',
        body: `Toutes tes décisions majeures sont à jour dans leur relecture. Revenir sur ses choix pour vérifier qu'ils tiennent encore la route est une habitude rare — "celui qui écoute le conseil est sage" (Proverbes 12:15), y compris quand ce conseil vient de soi-même, à tête reposée. Continue à te donner ce temps de relecture.` });
    }
  }
  ...
  const qIdx = Math.floor(today.getMonth() / 3);
  const qStart = new Date(today.getFullYear(), qIdx * 3, 1);
  const daysIntoQuarter = days(qStart) * -1;
  const hasRevueThisQuarter = (revues || []).some(r => new Date(r.date) >= qStart && new Date(r.date) <= today);
  const prevQStart = new Date(qIdx === 0 ? today.getFullYear() - 1 : today.getFullYear(), ((qIdx + 3) % 4) * 3, 1);
  const prevQEnd = new Date(qStart.getTime() - 1);
  const hadRevuePrevQuarter = (revues || []).some(r => new Date(r.date) >= prevQStart && new Date(r.date) <= prevQEnd);
  if (daysIntoQuarter > 45 && !hasRevueThisQuarter) {
    conseils.push({ id: 'revue_retard', priority: 55, tone: 'orientation', category: 'Vision', title: 'S\'arrêter pour mieux avancer',
      body: `Le trimestre est déjà bien entamé et aucune revue n'a encore été faite. On avance parfois si vite qu'on ne prend plus le temps de vérifier si la direction est la bonne (Habacuc 2:2 invite à "écrire la vision" et à la garder lisible). Bloque un moment calme cette semaine, même 20 minutes, pour faire ce point.` });
  } else if (hasRevueThisQuarter && hadRevuePrevQuarter) {
    conseils.push({ id: 'revue_constante', priority: 10, tone: 'encouragement', category: 'Vision', title: 'Une vision qu\'on entretient',
      body: `Deux trimestres de suite avec une vraie revue — c'est le genre de rythme qui, avec le temps, change vraiment une trajectoire. Peu de gens prennent ce recul régulièrement. Continue : la vision "s'accomplit au temps fixé" (Habacuc 2:3), et se relire régulièrement, c'est rester attentif à ce temps.` });
  }

  // --- 5. Provisions ---
  (provisions || []).forEach(p => {
    if (p.type === 'projet' && p.targetDate) {
      const toDue = Math.floor((new Date(p.targetDate) - today) / 86400000);
      if (toDue >= 0 && toDue <= 30 && (p.reserveCurrent || 0) < (p.targetAmount || 0)) {
        conseils.push({ id: `provision_retard_${p.id}`, priority: 60, tone: 'orientation', category: 'Provisions', title: 'Anticiper plutôt que subir',
          body: `Provision « ${p.name} » : la réserve n'est pas encore complète à ${toDue} jours de l'échéance. Prévoir à l'avance, c'est justement ce que le sage fait "avant que les jours mauvais viennent" (Ecclésiaste 11:2 — répartir en plusieurs parts, sans savoir ce qui arrivera). S'il est trop tard pour compléter entièrement, vaut peut-être mieux ajuster l'échéance ou le montant que de puiser ailleurs dans l'urgence.` });
      } else if ((p.reserveCurrent || 0) >= (p.targetAmount || 0) && toDue > 0) {
        conseils.push({ id: `provision_avance_${p.id}`, priority: 12, tone: 'encouragement', category: 'Provisions', title: 'Prévoyance accomplie',
          body: `Provision « ${p.name} » : l'objectif est atteint avec ${toDue} jours d'avance. C'est exactement l'esprit de la fourmi citée en exemple (Proverbes 6:6-8) — elle prépare sans attendre la pression. Cette avance te donne une marge : à toi de voir si elle mérite d'être utilisée pour souffler, ou pour préparer la suivante.` });
      }
    } else if (p.type !== 'projet') {
      const dueMonth = Number(p.dueMonth || 12);
      const monthsToDue = (dueMonth - 1 - today.getMonth() + 12) % 12;
      if (monthsToDue <= 1 && (p.reserveCurrent || 0) < (p.annualAmount || 0)) {
        conseils.push({ id: `provision_retard_rec_${p.id}`, priority: 60, tone: 'orientation', category: 'Provisions', title: 'Anticiper plutôt que subir',
          body: `Provision « ${p.name} » : la réserve n'est pas encore complète à l'approche de son échéance annuelle. Prévoir à l'avance, c'est justement ce que le sage fait "avant que les jours mauvais viennent" (Ecclésiaste 11:2). S'il est trop tard pour tout compléter, un versement partiel maintenant vaut mieux qu'un manque total le moment venu.` });
      } else if ((p.reserveCurrent || 0) >= (p.annualAmount || 0)) {
        conseils.push({ id: `provision_ok_rec_${p.id}`, priority: 8, tone: 'encouragement', category: 'Provisions', title: 'Prévoyance accomplie',
          body: `Provision « ${p.name} » : entièrement constituée avant son échéance. C'est l'esprit de la fourmi (Proverbes 6:6-8), qui prépare sans attendre la pression du moment.` });
      }
    }
  });

  // --- 6. Dettes ---
  const totalDettes = (debts || []).reduce((s, d) => s + Number(d.currentBalance || 0), 0);
  const sansPlan = (debts || []).filter(d => Number(d.currentBalance || 0) > 0 && Number(d.monthlyPayment || 0) === 0);
  if (sansPlan.length > 0) {
    conseils.push({ id: 'dette_sans_plan', priority: 58, tone: 'orientation', category: 'Dettes', title: 'Un plan vaut mieux qu\'une intention',
      body: `« ${sansPlan[0].name || sansPlan[0].nom || 'Une dette'} » n'a pas de mensualité définie pour la résorber. "Le sage prévoit le mal et se met à l'abri" (Proverbes 22:3) — même un petit montant fixe régulier change la trajectoire d'une dette bien plus qu'une bonne intention sans échéancier. Fixe un chiffre, même modeste, cette semaine.` });
  } else if ((debts || []).length > 0 && totalDettes === 0) {
    conseils.push({ id: 'dette_libre', priority: 5, tone: 'encouragement', category: 'Dettes', title: 'Libre de toute dette',
      body: `Aucune dette en cours actuellement. "Ne devez rien à personne, si ce n'est de vous aimer les uns les autres" (Romains 13:8) — cette liberté est une vraie marge de manœuvre pour donner et pour provisionner. Elle mérite d'être protégée : reste vigilant avant d'en recontracter une par confort.` });
  }

  // --- 7. Objectifs ---
  const objEnRetard = (objectifs || []).filter(o => o.statut === 'en cours' && o.echeance && o.echeance < todayIso);
  if (objEnRetard.length > 0) {
    conseils.push({ id: 'objectif_retard', priority: 50, tone: 'orientation', category: 'Vision', title: 'Réajuster n\'est pas abandonner',
      body: `Objectif « ${objEnRetard[0].titre} » : l'échéance est dépassée et il est toujours "en cours". Ce n'est pas un échec de repousser une échéance qui était irréaliste — c'est un échec de ne jamais la revoir. "Sans révélation, le peuple est sans frein" (Proverbes 29:18) : redonne une date à cet objectif, même lointaine, plutôt que de le laisser flotter.` });
  } else {
    const nonAbandonnes = (objectifs || []).filter(o => o.statut !== 'abandonné');
    const atteints = nonAbandonnes.filter(o => o.statut === 'atteint');
    if (nonAbandonnes.length >= 3 && atteints.length / nonAbandonnes.length >= 0.5) {
      conseils.push({ id: 'objectif_bon_taux', priority: 18, tone: 'encouragement', category: 'Vision', title: 'Une vision qui se concrétise',
        body: `${atteints.length} objectif${atteints.length > 1 ? 's' : ''} atteint${atteints.length > 1 ? 's' : ''} sur ${nonAbandonnes.length} — plus de la moitié. Se fixer un but est facile ; le mener à terme l'est moins. "Achève ce que tu as commencé" (2 Corinthiens 8:11) : ce que tu vis là, continue à le cultiver.` });
    }
  }

  // --- 8. Relations ---
  const enRetard = (contacts || []).filter(c => days(c.dernierContact) > c.frequenceCible);
  if (enRetard.length >= 2) {
    conseils.push({ id: 'contacts_retard', priority: 45, tone: 'orientation', category: 'Relations', title: 'Ce qui se néglige se dissout',
      body: `${enRetard.length} personnes à recontacter selon le rythme que tu t'étais fixé. Une relation ne se rompt rarement d'un coup — elle s'éteint par petites absences répétées. "Ne néglige pas de faire le bien... quand tu en as le pouvoir" (Proverbes 3:27). Un seul message aujourd'hui, à une seule personne, suffit pour repartir.` });
  } else if ((contacts || []).length > 0 && enRetard.length === 0) {
    conseils.push({ id: 'contacts_a_jour', priority: 6, tone: 'encouragement', category: 'Relations', title: 'Des liens entretenus',
      body: `Tous tes contacts suivis sont à jour dans leur fréquence. Prendre soin des relations demande une attention aussi constante que les finances — et tu la donnes. "Un ami aime en tout temps" (Proverbes 17:17) : ce que tu fais là ne se voit pas dans un tableau, mais ça compte.` });
  }

  // --- 9. Lectures ---
  (lectures || []).forEach(l => {
    if (l.statut !== 'en cours') return;
    const logs = (lectureLogs || []).filter(e => e.lectureId === l.id);
    if (logs.length === 0) return;
    const lastDate = logs.reduce((max, e) => e.date > max ? e.date : max, logs[0].date);
    const silence = days(lastDate);
    if (silence >= 10) {
      conseils.push({ id: `lecture_pause_${l.id}`, priority: 25, tone: 'orientation', category: 'Croissance', title: 'Reprendre sans culpabilité',
        body: `« ${l.titre} » : en pause depuis ${silence} jours. Ce n'est pas une course — "celui qui néglige l'instruction méprise son âme, mais celui qui écoute la réprimande acquiert l'intelligence" (Proverbes 15:32) parle de constance, pas de vitesse. Reprends où tu t'es arrêté, même quelques pages suffisent pour relancer l'élan.` });
    }
  });
  const recemmentTermine = (lectures || []).find(l => {
    if (l.statut !== 'terminé') return false;
    const logs = (lectureLogs || []).filter(e => e.lectureId === l.id);
    if (logs.length === 0) return false;
    const lastDate = logs.reduce((max, e) => e.date > max ? e.date : max, logs[0].date);
    return days(lastDate) <= 14;
  });
  if (recemmentTermine) {
    conseils.push({ id: `lecture_termine_${recemmentTermine.id}`, priority: 14, tone: 'encouragement', category: 'Croissance', title: 'Une graine de plus semée',
      body: `« ${recemmentTermine.titre} » : terminé récemment. S'instruire, c'est grandir en discernement (Proverbes 9:9) — chaque livre achevé est un petit dépôt qui, avec le temps, façonne le jugement. Qu'est-ce que cette lecture t'a donné envie de changer concrètement ?` });
  }

  return conseils.sort((a, b) => b.priority - a.priority);
}

function ConseilTab({ settings, transactions, disciplineLogs, contacts, decisions, revues, provisions, objectifs, lectures, lectureLogs, debts }) {
  const conseils = useMemo(() => computeConseils({ settings, transactions, disciplineLogs, contacts, decisions, revues, provisions, objectifs, lectures, lectureLogs, debts }), [settings, transactions, disciplineLogs, contacts, decisions, revues, provisions, objectifs, lectures, lectureLogs, debts]);
  return (
    <div>
      <h2 style={{ fontSize: 16, fontWeight: 800, color: C.heading, marginBottom: 4 }}>Conseil</h2>
      <p style={{ fontSize: 11.5, color: C.fade, marginBottom: 16 }}>Un éclairage tiré de tes propres données — dans l'ordre de ce qui mérite le plus ton attention aujourd'hui.</p>
      {conseils.length === 0 && (
        <Card><p style={{ fontSize: 12.5, color: C.fade }}>Rien de particulier à signaler pour l'instant — continue simplement à noter tes données au fil de l'eau.</p></Card>
      )}
      <div style={{ display: 'grid', gap: 12 }}>
        {conseils.map(c => (
          <Card key={c.id} style={{ borderLeft: `3px solid ${c.tone === 'encouragement' ? C.green : C.gold}` }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
              <span style={{ fontSize: 10, fontWeight: 700, color: C.fade, textTransform: 'uppercase', letterSpacing: 0.4 }}>{c.category}</span>
              {c.tone === 'encouragement' && <span style={{ fontSize: 10, fontWeight: 700, color: C.green }}>🎉 Encouragement</span>}
            </div>
            <div style={{ fontSize: 14, fontWeight: 800, color: C.ink, marginBottom: 6 }}>{c.title}</div>
            <div style={{ fontSize: 12.5, color: C.ink, lineHeight: 1.5 }}>{c.body}</div>
          </Card>
        ))}
      </div>
    </div>
...
function ReminderModal({ reminders, onClose }) {
  const iconFor = { discipline: Sparkles, contact: Users, decision: Compass, vision: Compass };
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(11,47,78,0.55)', zIndex: 60, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: C.surface, borderRadius: 16, padding: 20, maxWidth: 380, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
          <div style={{ fontSize: 16, fontWeight: 800, color: C.heading }}>Quelques rappels</div>
          <IconBtn onClick={onClose}><X size={18} /></IconBtn>
        </div>
        <div style={{ fontSize: 11.5, color: C.fade, marginBottom: 14 }}>Des invitations, pas des reproches — à toi de voir ce qui mérite ton attention aujourd'hui.</div>
        <div style={{ display: 'grid', gap: 8, marginBottom: 16 }}>
          {reminders.map((r, i) => {
            const Icon = iconFor[r.icon] || Sparkles;
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, background: C.cream, borderRadius: 10, padding: '10px 12px' }}>
                <Icon size={16} color={C.gold} style={{ flexShrink: 0 }} />
                <span style={{ fontSize: 12.5, color: C.ink }}>{r.text}</span>
              </div>
            );
          })}
        </div>
        <button onClick={onClose} style={{ width: '100%', background: C.navy, color: '#fff', border: 'none', borderRadius: 10, padding: '11px', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>Ne plus me le rappeler aujourd'hui</button>
      </div>
    </div>
  );
}

// Conseils concrets par catégorie de rappel, affichés sur la page d'accueil d'une notification.
const NOTIF_GUIDANCE = {
  discipline: { icon: 'discipline', label: 'Discipline', tip: "Prends 5 minutes pour t'y remettre aujourd'hui — la régularité compte plus que la performance." },
  discipline_streak: { icon: 'discipline', label: 'Discipline', tip: "Continue sur cette lancée : chaque jour tenu renforce l'habitude." },
  contact: { icon: 'contact', label: 'Relation', tip: "Un message simple suffit pour renouer le contact aujourd'hui." },
  decision: { icon: 'decision', label: 'Décision', tip: "Relis cette décision à tête reposée avant qu'elle ne s'éloigne de ta mémoire." },
  vision_revue: { icon: 'vision', label: 'Vision', tip: "Prends un moment calme pour faire le point sur ce trimestre." },
  vision_objectif: { icon: 'vision', label: 'Vision', tip: "Réajuste l'échéance ou avance d'un petit pas aujourd'hui." },
  provision: { icon: 'vision', label: 'Provision', tip: "Une petite cotisation maintenant évite la pression au dernier moment." },
  lecture: { icon: 'vision', label: 'Croissance', tip: "Reprends où tu t'es arrêté, même quelques pages suffisent." },
  dime: { icon: 'vision', label: 'Royaume', tip: "Un ajustement avant la fin du mois te garde aligné avec ton objectif." },
  generic: { icon: 'vision', label: 'Point du jour', tip: "Un rapide passage en revue suffit — pas besoin de tout faire d'un coup." },
};

// Page d'accueil affichée quand on arrive depuis une notification : résume le(s) rappel(s),
// donne un conseil concret par catégorie, puis propose d'aller directement à l'action.
function NotificationLandingModal({ items, summary, onNavigate, onClose }) {
  const iconFor = { discipline: Sparkles, contact: Users, decision: Compass, vision: Compass };
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(11,47,78,0.55)', zIndex: 60, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: C.surface, borderRadius: 16, padding: 20, maxWidth: 400, width: '100%', maxHeight: '85vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
          <div style={{ fontSize: 16, fontWeight: 800, color: C.heading }}>{summary ? 'Ton résumé de la semaine' : 'Ton rappel'}</div>
          <IconBtn onClick={onClose}><X size={18} /></IconBtn>
        </div>

        {summary ? (
          <div style={{ fontSize: 12.5, color: C.ink, whiteSpace: 'pre-wrap', background: C.cream, borderRadius: 10, padding: '12px 14px', marginTop: 12, marginBottom: 16 }}>
            {summary}
          </div>
        ) : (
          <div style={{ display: 'grid', gap: 10, marginTop: 12, marginBottom: 16 }}>
            {(items || []).map((it, i) => {
              const g = NOTIF_GUIDANCE[it.type] || NOTIF_GUIDANCE.generic;
              const Icon = iconFor[g.icon] || Sparkles;
              return (
                <div key={i} style={{ background: C.cream, borderRadius: 10, padding: '12px 14px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                    <Icon size={15} color={C.gold} style={{ flexShrink: 0 }} />
                    <span style={{ fontSize: 10.5, fontWeight: 700, color: C.fade, textTransform: 'uppercase', letterSpacing: 0.3 }}>{g.label}</span>
                  </div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: C.ink, marginBottom: 4 }}>{it.text}</div>
                  <div style={{ fontSize: 11.5, color: C.fade, marginBottom: 10 }}>{g.tip}</div>
                  <button
                    onClick={() => onNavigate(it.tab)}
                    style={{ background: 'none', border: `1px solid ${C.navy}`, color: C.navy, borderRadius: 8, padding: '6px 12px', fontWeight: 700, fontSize: 11.5, cursor: 'pointer' }}
                  >
                    Y aller →
                  </button>
                </div>
        ...
        {summary && (
          <button onClick={() => onNavigate('dashboard')} style={{ width: '100%', background: C.navy, color: '#fff', border: 'none', borderRadius: 10, padding: '11px', fontWeight: 700, fontSize: 13, cursor: 'pointer', marginBottom: 8 }}>
            Voir mon tableau de bord
          </button>
        )}
        <button onClick={onClose} style={{ width: '100%', background: 'none', border: 'none', color: C.fade, padding: '8px', fontWeight: 600, fontSize: 12.5, cursor: 'pointer' }}>Fermer</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      {monthIdx > 0 && (() => {
        ...
        for (let m = 0; m <= monthIdx; m++) {
          const score = computeIndiceForMonth({ settings, transactions, debts, timeLogs, healthLogs, decisions, objectifs, revues, monthIdx: m, year });
          trend.push({ mois: MONTHS_FR[m].slice(0,3), score: Math.round(score) });
        }
        return (
          <>
            <SectionTitle sub="Ta trajectoire depuis le début de l'année — ce qui compte, c'est la tendance, pas un chiffre isolé.">Évolution ({year})</SectionTitle>
            <Card style={{ marginBottom: 16 }}>
              <div style={{ height: 140 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={trend} margin={{ top: 6, right: 8, left: -22, bottom: 0 }}>
                    <XAxis dataKey="mois" tick={{ fontSize: 10, fill: C.fade }} axisLine={{ stroke: C.line }} tickLine={false} />
                    <YAxis domain={[0,100]} tick={{ fontSize: 10, fill: C.fade }} axisLine={false} tickLine={false} />
                    <Tooltip formatter={(v) => v + ' /100'} />
                    <Line type="monotone" dataKey="score" stroke={C.gold} strokeWidth={2.5} dot={{ r: 3, fill: C.gold }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </Card>
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            {lectures.map(l => {
              ...
              return (
                <Card key={l.id}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ cursor: 'pointer', flex: 1, display: 'flex', alignItems: 'center', gap: 10 }} onClick={() => setExpandedId(isOpen ? null : l.id)}>
                      <div style={{ width: 36, height: 36, borderRadius: 10, background: C.cream, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        <BookOpen size={17} color={C.purple} />
                      </div>
                      <div style={{ minWidth: 0 }}>
                        <div style={{ fontSize: 13, fontWeight: 700 }}>{l.titre}</div>
                        <div style={{ fontSize: 11, color: C.fade }}>
                          {l.type}{l.auteur ? ` · ${l.auteur}` : ''}
                          {l.totalPages ? ` · ${total}/${l.totalPages} pages` : (total > 0 ? ` · ${total} pages lues` : '')}
                        </div>
                      </div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                      {l.totalPages > 0 && <RingProgress value={progressPct/100} size={30} strokeWidth={3} color={progressPct>=100?C.green:C.purple} />}
        ...
        <div>
          ...
          <div style={{ display: 'grid', gap: 10 }}>
            {(settings.growthHabits || []).map(h => {
              ...
              const streak = computeStreak(growthLogs, 'habitId', h.id);
  ...
  return (
    <div style={{ marginTop: 4 }}>
      <div style={{ marginBottom: 8 }}>
        <div style={{ fontSize: 10, color: C.fade, marginBottom: 3 }}>Note du jour</div>
        <div style={{ display: 'flex', gap: 8 }}>
          <TextInput placeholder="Ce qui a été concrètement pratiqué…" value={note} onChange={e => setNote(e.target.value)} style={{ flex: 1 }} />
          <MicButton onResult={t => setNote(v => v ? v + ' ' + t : t)} />
        </div>
      </div>
      <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
      ...
      {avecNote.length === 0 && (
        <div style={{ textAlign: 'center', padding: '14px 0 4px', color: C.fade }}>
          <GraduationCap size={20} style={{ opacity: 0.4, marginBottom: 4 }} />
          <div style={{ fontSize: 11 }}>Aucune note enregistrée pour l'instant.</div>
        </div>
      )}
    </div>
    ...
    <div>
      ...
      <div style={{ display: 'grid', gap: 8 }}>
        {withStatus.map(c => {
          ...
          <Card key={c.id}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div style={{ cursor: 'pointer', flex: 1, display: 'flex', gap: 10 }} onClick={() => setExpandedId(isOpen ? null : c.id)}>
                <div style={{
                  width: 38, height: 38, borderRadius: '50%', background: colorForName(c.nom), color: '#fff',
            ...
            {isOpen && (
              <div style={{ marginTop: 10, borderTop: `1px solid ${C.line}`, paddingTop: 10 }}>
                <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
                  ...
                  <MicButton onResult={t => setEchangeNote({ ...echangeNote, [c.id]: (echangeNote[c.id] ? echangeNote[c.id] + ' ' : '') + t })} />
...
const VAPID_PUBLIC_KEY = 'BKyPJsoKodQG1DwiodhAqXocDacYVqJ5cKc5prEX-x8ka9tZZef8Tu6tIYMkafv1itvWPVTdjavhbgIf9NgFTaw';
...
const REMINDER_HOUR_OPTIONS = [5, 6, 7, 8, 9, 10, 18, 19, 20, 21, 22];
...
function NotificationsSection() {
  ...
  async function refresh() {
    ...
    try {
      ...
      if (sub) {
        const { data } = await supabase.from('push_subscriptions').select('reminder_hour').eq('endpoint', sub.endpoint).maybeSingle();
        if (data && data.reminder_hour != null) setReminderHour(data.reminder_hour);
      }
  ...
  async function enable() {
    ...
    try {
      ...
      if (permission !== 'granted') { setError("Autorisation refusée dans le navigateur."); setBusy(false); return; }
      const reg = await navigator.serviceWorker.ready;
      ...
      const { error } = await supabase.from('push_subscriptions').upsert(
        { user_id: user.id, endpoint: json.endpoint, p256dh: json.keys.p256dh, auth: json.keys.auth, reminder_hour: reminderHour },
        { onConflict: 'endpoint' }
      );
  ...
  async function changeHour(newHour) {
    ...
    try {
      ...
      const sub = await reg.pushManager.getSubscription();
      if (sub) {
        await supabase.from('push_subscriptions').update({ reminder_hour: newHour }).eq('endpoint', sub.endpoint);
      }
    } catch (e) { /* pas bloquant : l'heure sera reproposée à la prochaine activation */ }
  }

  if (status === 'unsupported') {
    return <Card style={{ marginBottom: 18 }}><p style={{ fontSize: 12.5, color: C.fade }}>Les notifications ne sont pas prises en charge sur ce navigateur/appareil.</p></Card>;
  }
  return (
    <Card style={{ marginBottom: 18 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <div style={{ fontSize: 13, fontWeight: 700 }}>Rappels quotidiens</div>
          <div style={{ fontSize: 11, color: C.fade }}>{status === 'on' ? `Activés — vers ${reminderHour}h` : 'Désactivés'}</div>
        </div>
        {status === 'on' ? (
          <button onClick={disable} disabled={busy} style={{ background: 'none', border: `1px solid ${C.terracotta}`, color: C.terracotta, borderRadius: 8, padding: '8px 14px', fontWeight: 700, fontSize: 12.5, cursor: 'pointer' }}>Désactiver</button>
        ) : (
          <button onClick={enable} disabled={busy || status==='checking'} style={{ background: C.navy, color: '#fff', border: 'none', borderRadius: 8, padding: '8px 14px', fontWeight: 700, fontSize: 12.5, cursor: 'pointer' }}>Activer</button>
        )}
      </div>
      {status === 'on' && (
        <div style={{ marginTop: 12, display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: 11.5, color: C.fade }}>Heure du rappel :</span>
          <select
            value={reminderHour}
            onChange={(e) => changeHour(Number(e.target.value))}
            style={{ border: `1px solid ${C.fade}55`, borderRadius: 8, padding: '5px 8px', fontSize: 12.5, color: C.ink, background: C.surface }}
          >
            {REMINDER_HOUR_OPTIONS.map((h) => <option key={h} value={h}>{h}h</option>)}
          </select>
        </div>
      )}
      {error && <div style={{ color: C.terracotta, fontSize: 12, marginTop: 8 }}>{error}</div>}
    </Card>
  );
}

function FoyerSection() {
  const [state, setState] = useState({ loading: true }); // { loading } | { none: true } | { foyerId, joinCode, memberCount }
  ...
  async function refresh() {
    ...
    try {
      const { data, error } = await supabase.rpc('my_foyer');
      if (error) throw error;
      if (!data || data.length === 0) { setState({ none: true }); return; }
      const row = data[0];
      setState({ foyerId: row.foyer_id, joinCode: row.join_code, memberCount: Number(row.member_count) || 1 });
    } catch (e) {
      setState({ none: true });
    }
  }
  ...
  async function createFoyer() {
    setBusy(true); setError('');
    const { error } = await supabase.rpc('create_foyer');
    if (error) { setError(error.message); setBusy(false); return; }
    sessionStorage.setItem('eden-reopen-settings','1'); window.location.reload();
  }
  async function joinFoyer() {
    if (!codeInput.trim()) return;
    setBusy(true); setError('');
    const { error } = await supabase.rpc('join_foyer', { code: codeInput.trim() });
    if (error) { setError(error.message); setBusy(false); return; }
    sessionStorage.setItem('eden-reopen-settings','1'); window.location.reload();
  }
  async function leaveFoyer() {
    setBusy(true); setError('');
    const { data: { user } } = await supabase.auth.getUser();
    const { error } = await supabase.from('foyer_members').delete().eq('user_id', user.id);
    if (error) { setError(error.message); setBusy(false); return; }
    sessionStorage.setItem('eden-reopen-settings','1'); window.location.reload();
  }

  if (state.loading) return <Card style={{ marginBottom: 18 }}><p style={{ fontSize: 12.5, color: C.fade }}>Chargement…</p></Card>;

  if (state.none) {
    return (
      <Card style={{ marginBottom: 18 }}>
        <PrimaryButton onClick={createFoyer} disabled={busy}><Plus size={15}/> Créer un foyer</PrimaryButton>
        <div style={{ fontSize: 11, color: C.fade, textAlign: 'center', margin: '10px 0' }}>— ou —</div>
        <div style={{ display: 'flex', gap: 8 }}>
          <TextInput placeholder="Code reçu de ton conjoint(e)" value={codeInput} onChange={e => setCodeInput(e.target.value.toUpperCase())} style={{ flex: 1 }} />
          <button onClick={joinFoyer} disabled={busy || !codeInput.trim()} style={{ background: C.navy, color: '#fff', border: 'none', borderRadius: 8, padding: '0 14px', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>Rejoindre</button>
        </div>
        {error && <div style={{ color: C.terracotta, fontSize: 12, marginTop: 8 }}>{error}</div>}
      </Card>
    );
  ...
  return (
    <Card style={{ marginBottom: 18 }}>
      <Row label="Membres" value={`${state.memberCount} / 2`} />
      {state.memberCount < 2 && (
        <>
          <div style={{ fontSize: 11, color: C.fade, marginTop: 8, marginBottom: 4 }}>Code à partager avec ton conjoint(e) :</div>
          <div style={{ fontSize: 22, fontWeight: 800, letterSpacing: 2, color: C.navy, fontFamily: FONT_MONO, textAlign: 'center', background: C.cream, borderRadius: 8, padding: '10px 0' }}>{state.joinCode}</div>
        </>
      )}
      <button onClick={leaveFoyer} disabled={busy} style={{ marginTop: 12, width: '100%', background: 'none', border: `1px solid ${C.terracotta}`, color: C.terracotta, borderRadius: 8, padding: '9px', fontWeight: 700, fontSize: 12.5, cursor: 'pointer' }}>Quitter le foyer</button>
      {error && <div style={{ color: C.terracotta, fontSize: 12, marginTop: 8 }}>{error}</div>}
    </Card>
  );
}
...
function ParametresPanel({ settings, saveSettings, onClose, onExport, onImport, onChangeCurrency, userEmail, trash, onRestore, onPurgeOne, onPurgeAll }) {
  ...
  const MENU_ITEMS = [
    { id: 'compte', label: 'Compte, notifications & foyer', sub: 'Connexion, rappels, partage à deux', Icon: Users },
    { id: 'apparence', label: 'Apparence & devise', sub: 'Thème, devise, préférences générales', Icon: Sparkles },
    { id: 'finances', label: 'Comptes & catégories financières', sub: 'Comptes, groupes, catégories, types de dons', Icon: Wallet },
    { id: 'autres', label: 'Personnalisation des modules', sub: 'Disciplines, temps, santé, croissance, relations', Icon: LayoutGrid },
    { id: 'capitaux', label: 'Capitaux & modules actifs', sub: 'Activer ou désactiver des capitaux entiers', Icon: Compass },
    { id: 'donnees', label: 'Corbeille & sauvegarde', sub: 'Éléments supprimés, export/import', Icon: Download },
  ];

  return (
    <div style={{ position: 'fixed', inset: 0, background: C.cream, zIndex: 20, overflowY: 'auto', paddingBottom: 40 }}>
      <div style={{ background: C.navy, color: '#fff', padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          {view !== 'menu' && (
            <IconBtn onClick={() => setView('menu')} color="#fff"><ChevronLeft size={20} /></IconBtn>
          )}
          <span style={{ fontFamily: FONT_DISPLAY, fontSize: 18, fontWeight: 700 }}>{view === 'menu' ? 'Paramètres' : MENU_ITEMS.find(m => m.id === view)?.label}</span>
        </div>
          ...
          <div style={{ display: 'grid', gap: 10 }}>
            {MENU_ITEMS.map(item => (
              <Card key={item.id} style={{ cursor: 'pointer' }}>
                <div onClick={() => setView(item.id)} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 40, height: 40, borderRadius: 12, background: C.cream, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <item.Icon size={19} color={C.gold} />
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 700 }}>{item.label}</div>
                    <div style={{ fontSize: 11.5, color: C.fade }}>{item.sub}</div>
                  </div>
                  <ChevronRight size={16} color={C.fade} />
                </div>
              </Card>
            ))}
          </div>
      ...
      <div style={{ padding: 16 }}>
        ...
        <SectionTitle sub="Reçois une notification (même app fermée) s'il y a au moins un rappel à consulter — jamais plus d'une fois par jour.">Notifications</SectionTitle>
        <NotificationsSection />

        <SectionTitle sub="Partage tes Mouvements, Provisions, Dettes et Objectifs avec ton conjoint(e) — les deux personnes voient et modifient les mêmes données partagées. Tout le reste (Royaume, Sagesse, Santé, Croissance, Temps, Relations) reste personnel.">Foyer (partage à deux)</SectionTitle>
        <FoyerSection />
...
const CAPITALS = [
  ...
  { id: 'spirituel', name: 'Capital spirituel', icon: Crown, color: C.gold,
    modules: [
      ...
      { id: 'conseil', label: 'Conseil', icon: Sparkles },
    ] },
...
function ModulesLauncher({ onSelect, onClose, settings, currentTab }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 25 }}>
      ...
      <div
        onClick={onClose}
        style={{ position: 'fixed', inset: 0, background: 'rgba(11,47,78,0.45)', animation: 'edenSheetBackdropIn 0.18s ease-out' }}
      />
      <div style={{
...
function SearchPanel({ settings, transactions, debts, provisions, decisions, journal, objectifs, lectures, contacts, onNavigate, onClose }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: C.bg, zIndex: 26, overflowY: 'auto' }}>
...
// Décode le lien profond stocké dans l'historique (même format que celui envoyé par push).
function decodeNotifUrl(url) {
  try {
    const qs = url.split('?')[1] || '';
    const encoded = new URLSearchParams(qs).get('n');
...
function NotificationCenter({ onClose, onNavigate }) {
  ...
  async function openItem(n) {
    if (!n.read) {
      await supabase.from('notifications_log').update({ read: true }).eq('id', n.id);
      setItems(prev => prev.map(x => x.id === n.id ? { ...x, read: true } : x));
    }
    const decoded = decodeNotifUrl(n.url || '');
    if (decoded && decoded.items && decoded.items[0]) onNavigate(decoded.items[0].tab);
    else onNavigate('dashboard');
    onClose();
  }

  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(11,47,78,0.55)', zIndex: 60, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '60px 16px 20px' }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: C.surface, borderRadius: 16, padding: 16, maxWidth: 420, width: '100%', maxHeight: '80vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          <div style={{ fontSize: 15, fontWeight: 800, color: C.heading }}>Notifications</div>
          <IconBtn onClick={onClose}><X size={18} /></IconBtn>
        </div>
        {loading && <div style={{ fontSize: 12, color: C.fade, padding: 12 }}>Chargement…</div>}
        {!loading && items.length === 0 && <div style={{ fontSize: 12, color: C.fade, padding: 12 }}>Aucune notification pour l'instant.</div>}
        <div style={{ display: 'grid', gap: 6 }}>
          {items.map(n => (
            <button key={n.id} onClick={() => openItem(n)} style={{
              textAlign: 'left', border: 'none', cursor: 'pointer',
              background: n.read ? C.cream : `${C.gold}22`,
              borderRadius: 10, padding: '10px 12px',
            }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: C.ink }}>{n.title}</div>
              <div style={{ fontSize: 11.5, color: C.fade, marginTop: 2 }}>{n.body}</div>
              <div style={{ fontSize: 10, color: C.fade, marginTop: 4, opacity: 0.8 }}>
...
export default function App() {
  ...
  useEffect(() => {
    if (loading) return;
    if (localStorage.getItem('eden-reminders-date') === todayISO()) return;
    const r = computeReminders({ settings, disciplineLogs, contacts, decisions, revues });
    if (r.length > 0) { setReminders(r); setShowReminders(true); }
    localStorage.setItem('eden-reminders-date', todayISO());
  }, [loading]);

  // Centre de notifications : historique + badge de non-lus.
  const [showNotifCenter, setShowNotifCenter] = useState(false);
  const [unreadNotifCount, setUnreadNotifCount] = useState(0);
  async function refreshUnreadCount() {
    const { count } = await supabase.from('notifications_log').select('id', { count: 'exact', head: true }).eq('read', false);
    setUnreadNotifCount(count || 0);
  }
  useEffect(() => { refreshUnreadCount(); }, []);

  const [notifLanding, setNotifLanding] = useState(null); // { items } | { summary } | null
  useEffect(() => {
    try {
      ...
      if (decoded && decoded.summary) setNotifLanding({ summary: decoded.summary });
      else if (decoded && Array.isArray(decoded.items)) setNotifLanding({ items: decoded.items });
      // Nettoie l'URL pour qu'un rechargement ne rouvre pas la même page.
      window.history.replaceState({}, '', window.location.pathname);
    } catch (e) { /* lien de notification invalide ou absent : on ignore */ }
  }, []);
  ...
  const [tab, setTabRaw] = useState(() => {
    try {
      ...
      const validTabs = ['dashboard','transactions','royaume','provisions','dettes','sagesse','conseil','vision','temps','sante','croissance','relations','intendance'];
      return validTabs.includes(urlTab) ? urlTab : 'dashboard';
    } catch (e) { return 'dashboard'; }
  });
  const [prevTab, setPrevTab] = useState(null);
  const [enteredViaModules, setEnteredViaModules] = useState(false);
  function navigateTab(id, opts = {}) {
    if (id === tab) return;
    setPrevTab(tab);
    setEnteredViaModules(!!opts.viaModules);
    setTabRaw(id);
  }
  function goBack() {
    if (enteredViaModules) { setShowModules(true); return; }
  ...
  const [showSettings, setShowSettings] = useState(() => {
    if (sessionStorage.getItem('eden-reopen-settings')) {
      sessionStorage.removeItem('eden-reopen-settings');
      return true;
    }
    return false;
  });
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ maxWidth: 460, margin: '0 auto', minHeight: '100vh', background: C.surface, boxShadow: '0 0 30px rgba(0,0,0,0.08)', position: 'relative', paddingBottom: 76 }}>
        ...
        {showNotifCenter && (
          <NotificationCenter
            onClose={() => { setShowNotifCenter(false); refreshUnreadCount(); }}
            onNavigate={(tabId) => navigateTab(tabId)}
          />
        )}
        {notifLanding && (
          <NotificationLandingModal
            items={notifLanding.items}
            summary={notifLanding.summary}
            onNavigate={(tabId) => { navigateTab(tabId); setNotifLanding(null); }}
            onClose={() => setNotifLanding(null)}
          />
        )}
      </div>
    </div>
